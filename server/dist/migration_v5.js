...
async function migrate() {
    ...
    `);
        ...
        const [columns] = await db_1.writePool.query('SHOW COLUMNS FROM products');
        const columnNames = columns.map(c => c.Field);
        if (!columnNames.includes('category_id')) {
            await db_1.writePool.query('ALTER TABLE products ADD COLUMN category_id VARCHAR(36) AFTER shop_id');
            console.log('Column "category_id" added to products.');
        }
        if (!columnNames.includes('supplier_id')) {
            await db_1.writePool.query('ALTER TABLE products ADD COLUMN supplier_id VARCHAR(36) AFTER category_id');
        ...
        try {
            await db_1.writePool.query('ALTER TABLE products ADD CONSTRAINT fk_product_category FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL');
            await db_1.writePool.query('ALTER TABLE products ADD CONSTRAINT fk_product_supplier FOREIGN KEY (supplier_id) REFERENCES suppliers(id) ON DELETE SET NULL');
