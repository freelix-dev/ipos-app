...
async function applyUpdates() {
    try {
        ...
        try {
            ...
            await db_1.writePool.query('ALTER TABLE orders DROP COLUMN user_name');
        ...
        try {
            ...
            await db_1.writePool.query('ALTER TABLE orders MODIFY COLUMN user_id VARCHAR(36)');
      ...
      `);
        ...
        for (const [id, name, email, password, role, lastLogin] of users) {
            await db_1.writePool.query('INSERT IGNORE INTO users (id, name, email, password, role, lastLogin) VALUES (?, ?, ?, ?, ?, ?)', [id, name, email, password, role, lastLogin]);
        }
