...
async function debugJoin() {
  try {
    ...
    const [users] = await readPool.query<RowDataPacket[]>('SELECT id, name, email FROM users');
    console.table(users);

    console.log('\n--- Checking Orders and their User IDs ---');
    const [orders] = await readPool.query<RowDataPacket[]>('SELECT id, order_no, user_id FROM orders LIMIT 10');
    console.table(orders);

    console.log('\n--- Testing JOIN Query ---');
    const [joined] = await readPool.query<RowDataPacket[]>(`
