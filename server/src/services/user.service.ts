...
export const getAllUsers = async (shopId?: string, ownerId?: string) => {
  let query = `
  ...
  return users.map(u => {
    ...
    if (u.shop_id) shopIdsSet.add(u.shop_id);
    if (u.assigned_shop_ids) {
      u.assigned_shop_ids.split(',').forEach((id: string) => shopIdsSet.add(id));
    }
    return {
      ...u,
...
export const createUser = async (userData: any) => {
  ...
  try {
    ...
    await connection.query(
      'INSERT INTO users (id, shop_id, owner_id, name, email, password, role) VALUES (?, ?, ?, ?, ?, ?, ?)',
      [id, primaryShopId, owner_id || null, name, email, password, role || 'user']
    );
...
export const findUserByEmailAndPassword = async (email: string, password: string) => {
  const [users] = await readPool.query<RowDataPacket[]>(
    `SELECT u.id, u.shop_id, u.owner_id, u.email, u.name, u.role, s.name as shop_name 
     FROM users u 
     LEFT JOIN shops s ON u.shop_id = s.id 
  ...
  if (user) {
    ...
    const [userShops] = await readPool.query<RowDataPacket[]>(
