...
export const getAllOrders = async () => {
  const [orders] = await readPool.query<RowDataPacket[]>(`
  ...
  try {
    ...
    for (const incomingOrder of ordersToProcess) {
      try {
        ...
        if (user_id) {
          const [userExists] = await connection.query<RowDataPacket[]>('SELECT id FROM users WHERE id = ?', [user_id]);
        ...
        if (existing.length > 0) {
          ...
          await connection.query(
            `UPDATE orders SET status = ?, remark = ?, user_id = ?, syncedAt = ? WHERE id = ?`, 
            [status, remark || null, user_id, syncedAt, id]
          );
          ...
          await connection.query(
            `INSERT INTO orders (id, date, total, status, currency, paymentMethod, itemsJson, amountReceived, changeAmount, remark, user_id, syncedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, 
            [id, new Date(date), total, status, currency, paymentMethod, items, amountReceived || 0, changeAmount || 0, remark || null, user_id, new Date(syncedAt)]
          );
