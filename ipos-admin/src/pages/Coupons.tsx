  ...
  return (
    <div className="animate-slide-up">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '48px' }}>
        <div>
          <h1 style={{ fontSize: '2.4rem', fontWeight: 900, letterSpacing: '-0.04em', color: 'var(--text-main)', marginBottom: '8px' }}>ບັດສ່ວນຫຼຸດ</h1>
          <p style={{ color: 'var(--text-muted)', fontSize: '1.1rem', fontWeight: 500 }}>ສ້າງລະຫັດຄູປອງພິເສດສຳລັບລູກຄ້າທີ່ຊື່ສັດຂອງທ່ານ</p>
        </div>
        <button onClick={() => setShowAddModal(true)} className="btn-primary" style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
          <Plus size={20} />
          <span>ສ້າງຄູປອງໃໝ່</span>
        </button>
      </div>
          ...
          {coupons.map((coupon) => (
            <div key={coupon.id} className="card-premium" style={{ background: '#fff', borderRadius: '24px', border: '1px solid var(--border)', overflow: 'hidden' }}>
              <div style={{ background: 'var(--primary)', padding: '24px', color: '#fff', textAlign: 'center' }}>
                ...
                <p style={{ fontSize: '0.8rem', opacity: 0.8, fontWeight: 700, marginTop: '4px' }}>{coupon.discount_type === 'percentage' ? `ສ່ວນຫຼຸດ ${coupon.discount_value}%` : `ສ່ວນຫຼຸດ ${coupon.discount_value} ₭`}</p>
              </div>
              
              <div style={{ padding: '24px' }}>
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '20px' }}>
                  <div style={{ padding: '12px', background: '#f8fafc', borderRadius: '16px', textAlign: 'center' }}>
                    <p style={{ fontSize: '0.65rem', fontWeight: 900, color: 'var(--text-muted)', marginBottom: '4px' }}>ຄົງເຫຼືອ</p>
                    <p style={{ fontWeight: 900, fontSize: '1rem' }}>{coupon.usage_limit - coupon.used_count}</p>
                  </div>
                  <div style={{ padding: '12px', background: '#f8fafc', borderRadius: '16px', textAlign: 'center' }}>
                    <p style={{ fontSize: '0.65rem', fontWeight: 900, color: 'var(--text-muted)', marginBottom: '4px' }}>ໝົດອາຍຸ</p>
                    <p style={{ fontSize: '0.65rem', fontWeight: 900, color: 'var(--text-muted)', marginBottom: '4px' }}>ວັນໝົດອາຍຸ</p>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                   <div style={{ display: 'flex', alignItems: 'center', gap: '8px', color: coupon.status === 'active' ? '#10b981' : '#ef4444', fontWeight: 800, fontSize: '0.8rem' }}>
                       ...
                       {coupon.status === 'active' ? 'ເປີດໃຊ້ງານ' : 'ປິດໃຊ້ງານ'}
                    </div>
      ...
      {showAddModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(10px)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 10000 }}>
          <div className="animate-scale-in" style={{ background: '#fff', width: '550px', borderRadius: '32px', padding: '40px', boxShadow: '0 25px 50px rgba(0,0,0,0.2)' }}>
            <h2 style={{ fontSize: '1.8rem', fontWeight: 900, marginBottom: '8px' }}>ສ້າງບັດສ່ວນຫຼຸດ</h2>
            <p style={{ color: 'var(--text-muted)', marginBottom: '32px', fontWeight: 500 }}>ອອກລະຫັດສ່ວນຫຼຸດພິເສດໃໝ່</p>

            <form onSubmit={handleCreate}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                <div>
                  <label className="input-label">ລະຫັດຄູປອງ</label>
                  <div style={{ display: 'flex', gap: '12px' }}>
                    <input type="text" value={newCoupon.code} onChange={e => setNewCoupon({...newCoupon, code: e.target.value})} className="input-premium" style={{ flex: 1, textTransform: 'uppercase' }} required placeholder="ຕົວຢ່າງ: WELCOME2024" />
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  <div>
                    <label className="input-label">ປະເພດສ່ວນຫຼຸດ</label>
                  ...
                  <div>
                    <label className="input-label">ມູນຄ່າສ່ວນຫຼຸດ</label>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '20px' }}>
                  <div>
                    <label className="input-label">ຍອດຊື້ຂັ້ນຕ່ຳ (₭)</label>
                  ...
                  <div>
                    <label className="input-label">ຈຳນວນທີ່ໃຊ້ໄດ້</label>
                ...
                <div>
                    <label className="input-label">ວັນໝົດອາຍຸ</label>
                ...
                <div style={{ display: 'flex', gap: '20px', marginTop: '12px' }}>
                  ...
                  <button type="submit" className="btn-primary" style={{ flex: 2 }}>ສ້າງບັດສ່ວນຫຼຸດ</button>
                </div>
              </div>
            </form>
          </div>
